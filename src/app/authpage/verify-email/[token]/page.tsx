...
// Unwrap async `params` using React's `use` helper
const resolvedParams = reactUse(params as any) as { token: string }

useEffect(()=>{
	(async()=>{
		try{
			const res = await fetch('/api/auth/verify-email',{method:'POST', body: JSON.stringify({ token: resolvedParams.token }), headers:{'Content-Type':'application/json'}})
			if (res.ok) {
